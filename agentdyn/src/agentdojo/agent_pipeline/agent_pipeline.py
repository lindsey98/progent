...
def _add_repo_to_syspath(repo_name: str, src_subdir: str | None = None) -> Path:
    ...
    if repo_name == "progent" and (_workspace_root() / "secagent").exists():
        # This checkout lives inside the progent repo itself (<progent>/agentdyn):
        # the workspace root is the progent repo, with `secagent` importable from it.
        candidate_repo_paths.append(_workspace_root())
